...
loss_weight_np *=10
train_loss_weight_np = np.array([0.03163512, 0.00024158, 0.00703378, 1e6], np.float32)
train_loss_weight_np = 1.0/train_loss_weight_np             
train_loss_weight_np *=(4.0/np.sum(train_loss_weight_np))
train_loss_weight_np *=10
main_disk_path = '..'
input_data_folder_path    = '../0_data/' + target_city 
input_n_data_folder_path  = main_disk_path + '/0_data/' + target_city + 'n'
out_dir                   = 'output'
...
num_channel_2  = num_channel_2_src*2
...
other_city_list = ['R1', 'R2', 'R3', 'R7', 'R8']
other_city_list.remove(target_city)
num_other_city = 4
assert len(other_city_list) == num_other_city
...
if 1:  
  ...
  for i, day in enumerate(all_days):
    if i < num_val_case_begin or i >= num_val_case_end:
      ...
      val_days.append(day)
  #print('TRAIN_VAL_INDEX / TRAIN_VAL_RATIO:\t', TRAIN_VAL_INDEX, ' / ',  TRAIN_VAL_RATIO, '\t', )
  #print('all_days:',   len(all_days),   '\t', 
        #'train_days:', len(train_days), '\t',
        #'train_days(other city ', other_city_list[0], '):\t', len(other_city_train_days_list[0]), '\t',
        #'train_days(other city ', other_city_list[1], '):\t', len(other_city_train_days_list[1]), '\t',
        #'train_days(other city ', other_city_list[2], '):\t', len(other_city_train_days_list[2]), '\t',
        #'train_days(other city ', other_city_list[3], '):\t', len(other_city_train_days_list[3]), '\t',
...
if 1:
  ...
  missing_filepath = os.path.join(input_n_data_folder_path, 'missing.txt')
  
  num_line=0
  with open(missing_filepath) as missing_file:
    content = missing_file.readlines()
    ...
    for line in content:
      ...
      if (d,f) not in missing_dict:
        missing_dict[(d,f)] = list()
      missing_dict[(d,f)].append(c_type)
      num_line += 1
...
#print('continuous_output_info_list', '\t', continuous_output_info_list)
...
if __name__ == '__main__':
  ...
  train_loss_weight = torch.from_numpy(train_loss_weight_np).float().cuda()
  ...
  num_iteration_per_epoch = len(train_list)
  for i in range(len(other_train_list_list)):
    num_iteration_per_epoch += len(other_train_list_list[i])
  num_iteration_per_epoch /= BATCH_SIZE
  ...
  num_val_iteration_per_epoch = len(val_list) // BATCH_SIZE_VAL   
  ...
  for i in range(len(other_train_list_list)):
    np.random.shuffle(other_train_list_list[i])

  global_step = global_step_start
  epoch = float(global_step) / float(num_iteration_per_epoch)
  ...
  def get_data_and_label_from_other_city(day, frame_start, other_n_data_folder_path, ):
    ...
    input_data_2     = np.zeros((num_frame_before,    num_channel_2_src,    height, width), np.uint8)   
    input_data_4     = np.ones( (num_frame_before,    num_channel_2_src,    height, width), np.float32)   
    ...
    input_data_4[input_data_2==255] = 0
    input_data_2[input_data_2==255] = 0
    input_data_2 = input_data_2.astype(np.float32)
    input_data_2 /= 4   
    input_data_2 = np.moveaxis(input_data_2, 1, 0)  
    input_data_4 = np.moveaxis(input_data_4, 1, 0)  

    input_data_out = np.concatenate([input_data_out_1, input_data_out_3, input_data_2, input_data_4,], axis=0)
  ...
  def load_train_multithread():
    ...
    tt = 0
    tt_list = np.zeros((num_other_city),np.int32) 
    ...
    while True:
      ...
      random_city_i = np.random.randint(0,num_other_city)  # num_other_city+1)
      if random_city_i == num_other_city:
        ...
        other_n_data_folder_path  = main_disk_path + '/0_data/' + other_city + 'n'
  ...
  if 1:
    ...
    input_data         = np.zeros((BATCH_SIZE,     num_channel     * num_frame_before   + num_channel_static,     height, width), np.float32)
    ...
    val_input_data     = np.zeros((BATCH_SIZE_VAL, num_channel     * num_frame_before   + num_channel_static,     height, width), np.float32)    
    ...
    while True:
      ...
      loss_w       = train_loss_weight * loss_per_var
      loss         = torch.mean(loss_w) * 4.0
      sum_train_freq += 1
